...
scan_outputs = {}
scan_processes = {} # To keep track of running Wfuzz processes
scan_queues = {} # To store queues for real-time output

# Load examples from wfuzz_examples.txt
def load_examples(filename="wfuzz_examples.txt"):
    """Loads examples from a JSON file."""
    try:
        # Assuming wfuzz_examples.txt is in the same directory as app.py
...
@app.route('/')
def index():
    """Renders the main Wfuzz GUI HTML page."""
    return render_template('index.html')

# New endpoint to serve Wfuzz examples
@app.route('/get_examples', methods=['GET'])
def get_examples():
    """Returns the Wfuzz examples as JSON."""
    return jsonify(ALL_EXAMPLES)

@app.route('/generate_command', methods=['POST'])
def generate_command():
    """Generates the Wfuzz command based on form data."""
    data = request.json
    command_parts = ["wfuzz"]
    ...
    add_arg("-z", data.get('wordlist_payload_entry'))
    add_arg("--slice", data.get('slice_payload_entry'))
    add_arg("--script", data.get('script_entry'))
    ...
    # Fuzzing & Payloads
    encoders = data.get('encoders_entry', '').strip()
    if encoders:
        command_parts.append("-e")
        command_parts.append(shlex.quote(encoders))
    
    add_checkbox_arg("--iter", data.get('iterate_payloads_var'))
    add_checkbox_arg("-L", data.get('follow_redirects_var'))

    # Filters
    add_arg("--hc", data.get('hide_code_entry'))
    add_arg("--sc", data.get('show_code_entry'))
    add_arg("--hl", data.get('hide_lines_entry'))
    add_arg("--sl", data.get('show_lines_entry'))
    add_arg("--hw", data.get('hide_words_entry'))
    add_arg("--sw", data.get('show_words_entry'))
    add_arg("--hC", data.get('hide_chars_entry'))
    add_arg("--sC", data.get('show_chars_entry'))
    add_arg("--hs", data.get('hide_size_entry'))
    add_arg("--ss", data.get('show_size_entry'))

    # Output & Verbosity
    add_checkbox_arg("-v", data.get('verbose_var'))
    add_arg("-o", data.get('output_file_entry'))
    add_checkbox_arg("-q", data.get('quiet_var'))
    add_checkbox_arg("--show-all", data.get('show_all_var'))

    # HTTP Options
    add_arg("-p", data.get('proxy_entry'))
    add_arg("--proxy-chain", data.get('proxy_chain_entry'))
    add_arg("--timeout", data.get('timeout_entry'))
    add_arg("-s", data.get('delay_entry'))
    add_arg("-t", data.get('concurrency_entry'))
    add_arg("-A", data.get('user_agent_entry'))
    add_arg("-R", data.get('referer_entry'))
    add_arg("-u", data.get('authentication_entry'))
    add_arg("-b", data.get('cookies_entry'))
    add_checkbox_arg("--ss", data.get('ignore_ssl_errors_var'))

    # Advanced
    add_checkbox_arg("--calibration", data.get('calibration_var'))
    add_arg("--bh", data.get('blacklist_codes_entry'))
    add_checkbox_arg("-r", data.get('recursive_var'))
    add_arg("--rd", data.get('recursion_depth_entry'))
    add_checkbox_arg("--fs", data.get('follow_subdomain_var'))
    add_checkbox_arg("--reqfile", data.get('request_file_var'))
    add_arg("--reqfile-replace", data.get('reqfile_replace_entry'))
...
@app.route('/run_wfuzz', methods=['POST'])
def run_wfuzz():
    """
    Executes the Wfuzz command received from the frontend.
    ...
    # Ensure wfuzz is the command being run
    if command[0] != 'wfuzz':
        return jsonify({'status': 'error', 'message': 'Only Wfuzz commands are allowed.'}), 403

    # Check if wfuzz executable exists using shutil.which
    if shutil.which(command[0]) is None:
        return jsonify({'status': 'error', 'message': f"Wfuzz executable '{command[0]}' not found on the server. Please ensure Wfuzz is installed and accessible in the system's PATH."}), 500
    ...
    def _run_wfuzz_thread(cmd, q, scan_id_val):
        full_output_buffer = []
        try:
            ...
            final_status_line = f"\nWfuzz finished with exit code: {return_code}\n"
            ...
            error_msg = f"Error: '{cmd[0]}' command not found. Make sure Wfuzz is installed and in your system's PATH.\nSTATUS: Error\n"
            q.put(error_msg)
    ...
    # Start the Wfuzz process in a separate thread
    thread = threading.Thread(target=_run_wfuzz_thread, args=(command, output_queue, scan_id))
    thread.daemon = True
    thread.start()

    return jsonify({'status': 'running', 'scan_id': scan_id, 'message': 'Wfuzz scan started.'})

@app.route('/get_scan_output/<scan_id>', methods=['GET'])
def get_scan_output(scan_id):
    """
    Polls for real-time Wfuzz scan output.
...
def save_output():
    ...
    filename = data.get('filename', f'wfuzz_output_{uuid.uuid4()}.txt')
...
@app.route('/install_wfuzz', methods=['POST'])
def install_wfuzz():
    """
    Attempts to install Wfuzz on the server (Linux/Termux only).
    WARNING: This endpoint executes system commands with 'sudo' or 'pip'.
    ...
    if sys.platform.startswith('linux'):
        ...
        def _install_thread(q, current_scan_id, p_type):
            ...
            try:
                if p_type == 'termux':
                    # For Termux, pkg install python and then pip install wfuzz
                    update_python_command = shlex.split("pkg install python -y")
                    install_command = shlex.split("pip install wfuzz")
                    q.put("Detected Termux. Using 'pkg' for Python and 'pip' for Wfuzz installation.\n")
                elif p_type == 'linux':
                    # For Linux, ensure pip is installed, then pip install wfuzz
                    update_pip_command = shlex.split("sudo apt update -y")
                    install_pip_command = shlex.split("sudo apt install python3-pip -y")
                    install_command = shlex.split("pip install wfuzz")
                    q.put("Detected Linux. Using 'sudo apt' for pip and 'pip' for Wfuzz installation.\n")
                else:
                    q.put("Error: Unsupported platform type for installation.\n")
                    q.put("---INSTALL_COMPLETE_FAILURE---")
                    return

                # First, ensure Python/pip is ready
                if p_type == 'termux':
                    q.put(f"Executing: {' '.join(update_python_command)}\n")
                    process = subprocess.Popen(
                        update_python_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, universal_newlines=True
                    )
                    for line in iter(process.stdout.readline, ''):
                        q.put(line)
                    ...
                    if process.returncode != 0:
                        q.put(f"Python installation failed with exit code {process.returncode}\n")
                        raise subprocess.CalledProcessError(process.returncode, update_python_command, "".join(temp_buffer_thread), "")
                elif p_type == 'linux':
                    q.put(f"Executing: {' '.join(update_pip_command)}\n")
                    process = subprocess.Popen(
                        update_pip_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, universal_newlines=True
                    )
                    for line in iter(process.stdout.readline, ''):
                        q.put(line)
                        temp_buffer_thread.append(line)
                    process.wait()
                    if process.returncode != 0:
                        q.put(f"APT update failed with exit code {process.returncode}\n")
                        raise subprocess.CalledProcessError(process.returncode, update_pip_command, "".join(temp_buffer_thread), "")

                    q.put(f"\nExecuting: {' '.join(install_pip_command)}\n")
                    process = subprocess.Popen(
                        install_pip_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, universal_newlines=True
                    )
                    for line in iter(process.stdout.readline, ''):
                        q.put(line)
                        temp_buffer_thread.append(line)
                    process.wait()
                    if process.returncode != 0:
                        q.put(f"pip installation failed with exit code {process.returncode}\n")
                        raise subprocess.CalledProcessError(process.returncode, install_pip_command, "".join(temp_buffer_thread), "")
                ...
                if install_process.returncode != 0:
                    q.put(f"Wfuzz installation failed with exit code {install_process.returncode}\n")
                ...
                error_msg = f"Error: Command not found ({e}). Ensure necessary package managers/pip are installed and in PATH.\n"
                q.put(error_msg)
                q.put("---INSTALL_COMPLETE_FAILURE---")
        ...
        return jsonify({
            ...
            'message': f'Wfuzz installation for {platform_type} started. Polling for output...'
        })
    else:
        return jsonify({
            'status': 'error',
            'message': 'Wfuzz installation via this interface is only supported on Linux/Termux systems.',
...
if __name__ == '__main__':
    ...
    print(f"Wfuzz sub-app is starting on port {port}...") # Added for clarity in logs
