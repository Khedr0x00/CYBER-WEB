...
# In-memory storage for command outputs (for demonstration).
...
# Load examples from strings_examples.txt
def load_examples(filename="strings_examples.txt"):
    """Loads examples from a JSON file."""
    try:
        # Assuming strings_examples.txt is in the same directory as app.py
...
@app.route('/')
def index():
    """Renders the main Strings GUI HTML page."""
    return render_template('index.html')

# New endpoint to serve Strings examples
@app.route('/get_examples', methods=['GET'])
def get_examples():
    """Returns the Strings examples as JSON."""
    return jsonify(ALL_EXAMPLES)

@app.route('/generate_command', methods=['POST'])
def generate_command():
    """Generates the 'strings' command based on form data."""
    data = request.json
    command_parts = ["strings"]
    ...
    # Basic Options
    add_checkbox_arg("-a", data.get('all_sections_var'))
    add_checkbox_arg("-f", data.get('print_file_name_var'))
    
    min_length = data.get('min_length_entry')
    if min_length:
        command_parts.append(f"-n {shlex.quote(str(min_length))}")

    # Output Format
    offset_radix = data.get('offset_radix_var')
    if offset_radix == 'o':
        command_parts.append("-o")
    elif offset_radix in ['x', 'd']:
        command_parts.append(f"-t {shlex.quote(offset_radix)}")

    # Encoding
    encoding = data.get('encoding_var')
    if encoding:
        command_parts.append(f"-e {shlex.quote(encoding)}")
    
    # Target File (must be last)
    target_file_path = data.get('target_file_path_entry')
    if target_file_path:
        command_parts.append(shlex.quote(target_file_path))

    generated_cmd = " ".join(command_parts)
    return jsonify({'command': generated_cmd})

@app.route('/run_strings', methods=['POST'])
def run_strings():
    """
    Executes the 'strings' command received from the frontend.
    ...
    scan_id = str(uuid.uuid4()) # Unique ID for this command execution
    ...
    # Ensure 'strings' is the command being run
    if command[0] != 'strings':
        return jsonify({'status': 'error', 'message': 'Only "strings" commands are allowed.'}), 403

    # Check if strings executable exists using shutil.which
    if shutil.which(command[0]) is None:
        return jsonify({'status': 'error', 'message': f"Strings executable '{command[0]}' not found on the server. Please ensure 'strings' is installed and accessible in the system's PATH (usually part of binutils)."}), 500

    # Create a new queue for this command's real-time output
    output_queue = queue.Queue()
    command_queues[scan_id] = output_queue
    command_outputs[scan_id] = "" # Initialize full output storage

    def _run_strings_thread(cmd, q, scan_id_val):
        full_output_buffer = []
        try:
            ...
            final_status_line = f"\nStrings command finished with exit code: {return_code}\n"
            ...
            error_msg = f"Error: '{cmd[0]}' command not found. Make sure 'strings' is installed and in your system's PATH.\nSTATUS: Error\n"
            q.put(error_msg)
    ...
    # Start the 'strings' process in a separate thread
    thread = threading.Thread(target=_run_strings_thread, args=(command, output_queue, scan_id))
    thread.daemon = True
    thread.start()

    return jsonify({'status': 'running', 'scan_id': scan_id, 'message': 'Strings command started.'})

@app.route('/get_command_output/<scan_id>', methods=['GET'])
def get_command_output(scan_id):
    """
    Polls for real-time 'strings' command output.
    ...
    if not output_queue:
        ...
        final_status = command_outputs.get(scan_id + "_status") # Check for installation status
    ...
    if command_finished:
        ...
        final_output_content = command_outputs.get(scan_id, "Command/Installation completed, but output not fully captured.")
...
def save_output():
    ...
    filename = data.get('filename', f'strings_output_{uuid.uuid4()}.txt')
...
def upload_file():
    """Handles file uploads for the 'strings' command."""
...
@app.route('/install_strings', methods=['POST'])
def install_strings():
    """
    Attempts to install 'strings' (part of binutils) on the server (Linux/Termux only).
    ...
    if sys.platform.startswith('linux'):
        ...
        def _install_thread(q, current_scan_id, p_type):
            ...
            try:
                if p_type == 'termux':
                    ...
                    install_command = shlex.split("pkg install binutils -y")
                    q.put("Detected Termux. Using 'pkg' for installation.\n")
                elif p_type == 'linux':
                    update_command = shlex.split("sudo apt update -y")
                    install_command = shlex.split("sudo apt install binutils -y")
                ...
                # Then, install binutils (which contains strings)
        ...
        return jsonify({
            ...
            'message': f'Strings installation for {platform_type} started. Polling for output...'
        })
    else:
        return jsonify({
            'status': 'error',
            'message': 'Strings installation via this interface is only supported on Linux/Termux systems.',
...
if __name__ == '__main__':
    ...
    print(f"Strings sub-app is starting on port {port}...") # Added for clarity in logs
