...
scan_outputs = {}
scan_processes = {} # To keep track of running Skipfish processes
scan_queues = {} # To store queues for real-time output

# Load examples from skipfish_examples.txt
def load_examples(filename="skipfish_examples.txt"):
    """Loads examples from a JSON file."""
    try:
        # Assuming skipfish_examples.txt is in the same directory as app.py
...
@app.route('/')
def index():
    """Renders the main Skipfish GUI HTML page."""
    return render_template('index.html')

# New endpoint to serve Skipfish examples
@app.route('/get_examples', methods=['GET'])
def get_examples():
    """Returns the Skipfish examples as JSON."""
    return jsonify(ALL_EXAMPLES)

@app.route('/generate_command', methods=['POST'])
def generate_command():
    """Generates the Skipfish command based on form data."""
    data = request.json
    command_parts = ["skipfish"]
    ...
    # Authentication Tab
    if data.get('auth_type_select') == 'basic':
        add_arg("-A", f"{data.get('basic_auth_user_entry')}:{data.get('basic_auth_pass_entry')}")
    elif data.get('auth_type_select') == 'form':
        add_arg("-F", data.get('form_auth_url_entry'))
        add_arg("-f", data.get('form_auth_data_entry'))
    elif data.get('auth_type_select') == 'ntlm':
        add_arg("-N", f"{data.get('ntlm_auth_user_entry')}:{data.get('ntlm_auth_pass_entry')}")

    add_arg("-H", data.get('custom_headers_entry'))
    add_arg("-U", data.get('user_agent_entry'))
    add_arg("-R", data.get('referer_entry'))
    add_arg("-C", data.get('cookie_entry'))

    # Crawl/Dictionary Tab
    add_arg("-W", data.get('wordlist_file_entry'))
    add_arg("-D", data.get('crawl_domain_entry'))
    add_arg("-X", data.get('exclude_url_entry'))
    add_arg("-I", data.get('include_url_entry'))
    add_arg("-B", data.get('blacklist_regex_entry'))
    add_arg("-G", data.get('grep_regex_entry'))
    add_arg("-L", data.get('max_crawl_depth_entry'))
    add_arg("-P", data.get('max_children_entry'))
    add_arg("-Y", data.get('max_total_requests_entry'))
    add_checkbox_arg("--follow-redirects", data.get('follow_redirects_var'))
    add_checkbox_arg("--skip-form-autofill", data.get('skip_form_autofill_var'))
    add_checkbox_arg("--force-ssl", data.get('force_ssl_var'))

    # Fuzzing/Injection Tab
    add_arg("-m", data.get('fuzzing_mode_select'))
    add_arg("-F", data.get('custom_payloads_file_entry'))
    add_arg("-T", data.get('test_types_entry'))
    add_checkbox_arg("--blind-sql", data.get('blind_sql_var'))
    add_checkbox_arg("--xss", data.get('xss_var'))
    add_checkbox_arg("--lfi", data.get('lfi_var'))
    add_checkbox_arg("--rfi", data.get('rfi_var'))
    add_checkbox_arg("--cmd-inject", data.get('cmd_inject_var'))

    # Performance Tab
    add_arg("-S", data.get('scan_speed_select'))
    add_arg("-c", data.get('concurrent_connections_entry'))
    add_arg("-t", data.get('request_timeout_entry'))
    add_arg("-w", data.get('scan_delay_entry'))
    add_arg("-e", data.get('max_errors_entry'))

    # Output/Reporting Tab
    add_arg("-o", data.get('output_directory_entry'))
    add_checkbox_arg("-q", data.get('quiet_mode_var'))
    add_checkbox_arg("-v", data.get('verbose_mode_var'))
    add_checkbox_arg("-d", data.get('debug_mode_var'))
    add_checkbox_arg("--html", data.get('report_html_var'))
    add_checkbox_arg("--xml", data.get('report_xml_var'))
    add_checkbox_arg("--csv", data.get('report_csv_var'))
    add_checkbox_arg("--json", data.get('report_json_var'))

    # Advanced Tab
    add_arg("--config", data.get('config_file_entry'))
    add_arg("--proxy", data.get('proxy_entry'))
    add_arg("--bind-address", data.get('bind_address_entry'))
    add_arg("--cert", data.get('client_cert_entry'))
    add_arg("--cert-pass", data.get('client_cert_pass_entry'))
    add_checkbox_arg("--no-http-error-check", data.get('no_http_error_check_var'))
    add_checkbox_arg("--no-content-type-check", data.get('no_content_type_check_var'))
    add_checkbox_arg("--no-cookie-check", data.get('no_cookie_check_var'))
...
@app.route('/run_skipfish', methods=['POST'])
def run_skipfish():
    """
    Executes the Skipfish command received from the frontend.
    ...
    # Ensure skipfish is the command being run
    if command[0] != 'skipfish':
        return jsonify({'status': 'error', 'message': 'Only Skipfish commands are allowed.'}), 403

    # Check if skipfish executable exists using shutil.which
    if shutil.which(command[0]) is None:
        return jsonify({'status': 'error', 'message': f"Skipfish executable '{command[0]}' not found on the server. Please ensure Skipfish is installed and accessible in the system's PATH."}), 500
    ...
    def _run_skipfish_thread(cmd, q, scan_id_val):
        full_output_buffer = []
        try:
            ...
            final_status_line = f"\nSkipfish finished with exit code: {return_code}\n"
            ...
            error_msg = f"Error: '{cmd[0]}' command not found. Make sure Skipfish is installed and in your system's PATH.\nSTATUS: Error\n"
            q.put(error_msg)
    ...
    # Start the Skipfish process in a separate thread
    thread = threading.Thread(target=_run_skipfish_thread, args=(command, output_queue, scan_id))
    thread.daemon = True
    thread.start()

    return jsonify({'status': 'running', 'scan_id': scan_id, 'message': 'Skipfish scan started.'})

# Modified get_scan_output to handle 'skipfish_install' ID
@app.route('/get_scan_output/<scan_id>', methods=['GET'])
def get_scan_output(scan_id):
    """
    Polls for real-time Skipfish scan output.
...
def save_output():
    ...
    filename = data.get('filename', f'skipfish_output_{uuid.uuid4()}.txt')
...
@app.route('/install_skipfish', methods=['POST'])
def install_skipfish():
    """
    Attempts to install Skipfish on the server (Linux/Termux only).
    ...
    if sys.platform.startswith('linux'):
        ...
        def _install_thread(q, current_scan_id, p_type):
            ...
            try:
                if p_type == 'termux':
                    ...
                    install_command = shlex.split("pkg install skipfish -y")
                    q.put("Detected Termux. Using 'pkg' for installation.\n")
                elif p_type == 'linux':
                    update_command = shlex.split("sudo apt update -y")
                    install_command = shlex.split("sudo apt install skipfish -y")
        ...
        return jsonify({
            ...
            'message': f'Skipfish installation for {platform_type} started. Polling for output...'
        })
    else:
        return jsonify({
            'status': 'error',
            'message': 'Skipfish installation via this interface is only supported on Linux/Termux systems.',
...
if __name__ == '__main__':
    ...
    print(f"Skipfish sub-app is starting on port {port}...") # Added for clarity in logs
